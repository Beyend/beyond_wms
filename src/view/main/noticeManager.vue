...
export default {
  ...
  data() {
    return {
      brs: [{ name: "公告管理", to: { name: "noticeManager" } }],
      //查询参数
      queryMap: {
        pageNo: 1,
        pageSize: 10,
  ...
  methods: {
    ...
    toDeleted(row) {
      this.$confirm("是否确认删除该公告", "提示", {
        ...
        .then(() => {
          this.$post("noticeSave", { id: row.id, deleted: "Y" }).then(res => {
            this.getTableData();
            this.$message({ message: "删除公告成功", type: "success",center:true });
          });
        })
        .catch(() => {});
    },
    getTableData() {
      ...
      this.$post("noticeList", this.queryMap)
        .then(res => {
    ...
    toEdit(row) {
      this.$to({ name: "noticeEdit", query: row });
    },
