    ...
    <ui-layout :brs="brs">
        <ui-container>
            ...
            <el-table :data="tableData" border style="width:80%;">
                ...
                <el-table-column prop="integralCase" label="所需金币" width="150" align="center"></el-table-column>
                ...
                <el-table-column prop="payPrice" label="实际付款" width="100" align="center">
                  <!-- <template slot-scope="scope">
                                <span>{{scope.row.payPrice * scope.row.count}}</span>
                  </template> -->
                </el-table-column>
                ...
                <el-table-column prop="integralCase" label="金币" width="100" align="center"></el-table-column>
               ...
               <el-table-column prop="createTime" label="下单时间" width="120" align="center">
                ...
                <el-table-column prop="id" label="操作" width="300" fixed="right" align="center">
                    <template slot-scope="scope">
                        <!-- 未发货 已支付 -->
                        <el-button v-if="scope.row.sendStatus == 'N' && scope.row.payStatus == 'Y' && scope.row.deleted == 'N'" @click="toSend(scope.row)" type="text" size="small">发货</el-button>
                        <!-- 未支付 待发货 未发货  -->
                        <el-button v-if="scope.row.payStatus == 'N' && scope.row.status == 1 || scope.row.status == 2 && scope.row.sendStatus == 'N' && scope.row.deleted == 'N'" @click="toCancel(scope.row)" type="text" size="small">取消</el-button>
                        <!-- 退单 已完成的订单 点击返还金币 -->
                        <el-button v-if="scope.row.status == 4 && scope.row.deleted == 'N'" @click="toAfter(scope.row)" type="text" size="small">退单</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      brs: [{ name: "金币订单", to: { name: "orderList" } }],
      date: [],
      startTimeNull: null,
      endTimeNull: null,
      queryMap: {
    ...
    toCancel(row) {
      this.$confirm("是否确认取消该订单?", "提示", {
        ...
        type: "warning",
          ...
          this.$post("orderSave", { id: row.id, status: "7", applyStatus:"N",memberId:row.memberId }).then(res => {
            this.getTableData();
            this.$message({
              message: "操作成功",
              type: "success",
