                ...
                <el-form :inline="true" :model="formData" ref="formData" class="demo-form-inline" label-position="right" label-width="100px">
                    ...
                    <el-col>
                        <el-row>
                            <el-form-item label=" ">
                                <div class="margin20">
                                    <el-button type="primary" @click="autoRechargeSave" v-if="checkStatus">确认修改</el-button>
...
export default {
  ...
  data() {
    return {
      brs: [{ name: "编辑自动充值", to: { name: "autoRechargeEdit" } }],
      formData: {
        name:null,
        phone:null,
        money:null,
  ...
  methods: {
    ...
    autoRechargeSave() {
      ...
      this.$post("autoRechargeSave", this.formData)
        .then(res => {
          this.$message({ message: "编辑自动充值成功", type: "success", center: true });
