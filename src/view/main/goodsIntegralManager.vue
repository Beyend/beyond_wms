        ...
        <ui-container>
            ...
            <el-row>
                <el-table :data="tableData" border style="max-width:80%;" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column prop="integralDeduction" label="金币" width="80" align="center"></el-table-column>
...
export default {
  ...
  data() {
    return {
      brs: [{ name: "金币商品", to: { name: "goodsManager" } }],
      //查询参数
      queryMap: {
        pageNo: 1,
        pageSize: 10,
  ...
  methods: {
    ...
    //跳转新增页
    goodsInsert() {
      this.$to({ name: "goodsIntegralInsert"});
    },
    getTableData() {
      let loadingInstance = Loading.service({ fullscreen: true });
    ...
    toCheckInDetail(row) {
      // this.$to({ name: "goodsIntegralDetail", query: row ,queryMap : this.queryMap});
      this.$to({ name: "goodsIntegralDetail", query: {id:row.id,pageNo:this.queryMap.pageNo} });
    },
    changeStatus(row) {
      let visibtext = null;
      let status = null;
