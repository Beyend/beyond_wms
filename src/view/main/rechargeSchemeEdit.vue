                ...
                <el-form :inline="true" :model="formData" ref="formData" class="demo-form-inline" label-position="left" label-width="100px">
                    ...
                    <el-col>
                        <el-row>
                            <el-form-item label=" ">
                                <div class="margin20">
                                    <el-button type="primary" @click="rechargeSchemeSave">确认修改</el-button>
...
export default {
  ...
  data() {
    return {
      brs: [{ name: "编辑充值方案", to: { name: "rechargeSchemeEdit" } }],
      formData: {}
    };
  },
  mounted() {
  ...
  methods: {
    rechargeSchemeSave() {
      ...
      this.$post("rechargeSchemeSave", this.formData)
        .then(res => {
          this.$message({ message: "编辑充值方案成功", type: "success", center: true });
    ...
    getGoodsReviewDetail() {
      let loadingInstance = Loading.service({ fullscreen: true });
      this.$get("rechargeSchemeDetail", this.$route.query.id)
        .then(res => {
          this.formData = res;
          loadingInstance.close();
        })
