            ...
            <el-row>
                <el-form :inline="true" :model="formData" :rules="rules" ref="formData" class="demo-form-inline" label-position="center" label-width="100px">
                    ...
                    <el-row>
                        <el-form-item label=" ">
                            <div class="margin20">
                                <el-button type="primary" @click="addGoodsType">确认添加</el-button>
...
export default {
  ...
  data() {
    return {
      brs: [{ name: "添加商品分类", to: { name: "goodsTypeInsert" } }],
      formData: {
        parentId: 0,
        name: null,
        sort: null,
      ...
      this.$refs["formData"].validate(valid => {
        if (valid) {
          ...
          this.$post("goodsTypeSave", this.formData)
            .then(res => {
              this.$message({ message: "新增商品分类成功", type: "success" ,center:true});
              loadingInstance.close();
              this.$to({ name: "goodsTypeManager" });
            })
            .catch(() => {
              loadingInstance.close();
            });
